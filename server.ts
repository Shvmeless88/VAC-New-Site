          ...
          for (const pick of picks.slice(0, 5)) {
            try {
              ...
              const prompt = pick.kind === "interior"
                ...
                : `Image 1 is a fixed dealership showroom background plate (purple wall with white V logo, white wall, circular floor turntable). Image 2 is a dealer photo of a car. The turntable in image 1 is large. Produce a photorealistic composite: the EXACT car from image 2 parked centered ON the turntable (all four wheels inside the circle), seen from the SAME camera angle and framing as image 2 — do not rotate, re-pose, mirror or flip the car; if it is an exterior shot the nose should point LEFT toward the purple wall, matching the source photo's driver-side angle. Keep the turntable exactly as in image 1 (same size and position in frame; never enlarge, shrink, or move it) and park the car dead-centre INSIDE the circle — spanning about 65% of the platform width, every tire resting on the platform surface well inside the rim, with a wide ring of turntable visible all the way around the car. CRITICAL: the background must remain EXACTLY image 1 — identical walls, logo, floor, lighting, framing, nothing moved or restyled. Keep the car completely unchanged (body, wheels, badges, trim, colour). Adapt only the car's lighting/reflections to indoor studio light and add a natural soft shadow and subtle floor reflection under it. No text, no people, no extra objects.`;
              const genParts = [
                { text: prompt },
...
<rss xmlns:g="http://base.google.com/ns/1.0" version="2.0">
  ...
  // ---- Sales floor TV board: hidden URL + PIN gate ----
  // The board itself is salesfloor.html; deals data lives in Firestore doc
  // config/salesfloor (written by the morning PBS routine, later by the PBS
  // SFTP feed) so the TV updates with no deploy. PIN comes from the Cloud Run
...
if(r.ok)location.reload();else{if(p.value.length>=5){e.textContent='Wrong PIN';p.value='';}}});</script></body></html>`);
      ...
      app.get(dataPath, async (req, res) => {
        ...
        try {
          ...
          res.json({ deals: d?.deals || [], updatedAt: d?.updatedAt || null, adSpend: d?.adSpend || null, demand: d?.demand || null, repLeads: d?.repLeads || null, repDeals: d?.repDeals || null, aiSummary: d?.aiSummary || null });
        } catch (e: any) {
